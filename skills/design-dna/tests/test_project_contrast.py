...
FIXTURE_PATH = SKILL_ROOT / "tests" / "fixtures" / "project-contrast" / "ready-contract.json"
...
class ProjectContrastInitializerTests(unittest.TestCase):
    ...
    def test_bare_direction_capabilities_are_rejected_before_state_is_created(self) -> None:
        for capability, supported_profile in (
            ...
            with self.subTest(capability=capability), tempfile.TemporaryDirectory() as temporary:
                ...
                self.assertIn(supported_profile, error["message"])
                self.assertIn(
                    f"--evidence-capability {capability}",
                    error["message"],
                )
                self.assertFalse((root / ".design-dna").exists())

    def test_check_state_rejects_a_legacy_bare_direction_capability(self) -> None:
        for capability, supported_profile in (
            ("project-contrast", "--profile project-contrast"),
            ("direction-challenge", "--profile direction-challenge"),
        ):
            with self.subTest(capability=capability), tempfile.TemporaryDirectory() as temporary:
                root = Path(temporary)
                self.run_initializer(root, "--profile", "standard")
                state_path = root / ".design-dna" / "state.json"
                state = json.loads(state_path.read_text(encoding="utf-8"))
                state["evidence_contract"]["applicable_capabilities"].append(
                    capability
                )
                ...
                result = self.initializer_result(root, "--check-state")
                self.assertEqual(result.returncode, 1)
                payload = json.loads(result.stdout)
                self.assertTrue(
                    any(
                        supported_profile in failure
                        for failure in payload["failures"]
                    ),
                    payload["failures"],
                )

    def test_project_contrast_composes_connected_public_experience(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            ...
            self.assertTrue(
                {"project-contrast", "connected-public-experience"}.issubset(
                    result["records"]
                )
            )
            self.assertTrue(
            ...
            state = json.loads(
                (root / ".design-dna" / "state.json").read_text(
                    encoding="utf-8"
                )
            )
            self.assertTrue(
            ...
            self.assertTrue(
                (root / ".design-dna" / "project-contrast.json").is_file()
            )
            self.assertTrue(
                (root / ".design-dna" / "connected-public-experience.json")
                .is_file()
            )

    def test_ordinary_project_contrast_profile_selects_standard_and_no_direction_challenge(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            ...
            self.assertNotIn("direction-challenge", result["records"])
            self.assertNotIn("direction-challenge", result["assurance_profiles"])
            self.assertNotIn("direction-challenge", result["evidence_capabilities"])
            state = json.loads((root / ".design-dna" / "state.json").read_text(encoding="utf-8"))
            self.assertIn("project-contrast", state["records"])
            self.assertNotIn("direction-challenge", state["records"])
            self.assertFalse((root / ".design-dna" / "direction-challenge.json").exists())

    def test_first_use_guidance_distinguishes_ordinary_and_escalated_commands(self) -> None:
        guide = " ".join(
            (SKILL_ROOT / "references" / "quality" / "project-contrast.md")
            .read_text(encoding="utf-8")
            .split()
        )
        ordinary = "--profile project-contrast"
        escalation = "--profile showcase --trigger owner-recurrence-requirement"
        self.assertIn("ordinary unlike-recent-work request", guide)
        self.assertIn("does not create a Direction Challenge record", guide)
        self.assertIn("only for the explicit three-root recurrence escalation", guide)
        self.assertLess(guide.index(ordinary), guide.index(escalation))
