---
schema_version: 1
created_with: "__DESIGN_DNA_VERSION__"
classification: "internal"
evidence_contract: "proportional-evidence-v1"
---

# Claim ledger

<!-- proportional-evidence-v1 -->

Use this record when public copy, calculations, diagrams, or interactions
contain prices, quantities, durations, warranties, deadlines, performance
figures, regulated statements, categorical claims, or other exact-sounding
facts.

Do not treat `sample`, `demo`, or `concept` status as permission to invent
authority. Mark scenario values explicitly and record the assumptions that make
them useful.

## Claims

| ID | Exact claim or output | Type | Route/component | Status | Source and accountable owner | Scope, locale, jurisdiction, and assumptions | Reviewed / expires | Public treatment |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
|  |  | price, quantity, duration, warranty, availability, universal, regulated, calculation, or other |  | approved, scenario, pending, or prohibited |  |  |  | show, qualify, label, replace, defer, or omit |

## Calculators and derived outputs

| Output | Inputs | Formula or rule | Source of each input | Bounds and failure behavior | Maintenance owner |
| --- | --- | --- | --- | --- | --- |
|  |  |  |  |  |  |

## Closure

- Claims still pending or prohibited:
- Scenario values visibly labeled:
- Boundary map for scenario claims: context-free orientation, direct-entry
  routes, action-local labeling, and duplicate copy consolidated:
- Categorical words reviewed (`all`, `every`, `always`, `never`, `best`,
  `only`):
- Public copy checked against this ledger:
- Owner approval and date:
