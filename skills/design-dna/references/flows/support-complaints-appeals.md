# Support, complaints, disputes, and appeals
...
Before designing the flow, establish:
...
- Explain the route, purpose, required and optional evidence, deadlines,
  expected stages, possible outcomes, data use, and available assistance.
- Let people describe the problem in their own words without requiring them to
  understand internal categories.
- Collect only information needed for the next supported step. Allow saving,
  correction, attachment review, and a safe alternative when upload fails.
- Support language, access, representative or advocate, and non-digital needs
  that the real service can provide.
- Confirm receipt with a reference, submitted summary, timestamp, current
  status, next event, owner or channel, and correction path.

Submission is not receipt, assignment, investigation, decision, payment, refund,
or resolution. Name the state that the service can prove.

## Model the lifecycle

Use only supported states, such as draft, received, triaged, assigned, waiting
for the person, waiting internally, waiting for a third party, under review,
partially resolved, resolved, rejected, withdrawn, expired, escalated,
appealed, reopened, or closed.

For every state, define who owns it, what is known, what remains uncertain, the
next action, deadline or review point, and what happens when nothing changes.
Keep the same reference and history across transfer, escalation, appeal, and
reopening when lawful and technically supported.

## Preserve evidence and communication

- Give people a reviewable chronology, copies of their submissions and
  decisions, and a way to correct material errors.
- Preserve attachments, consent, authority, channel preference, access needs,
  and prior explanations only within authorized boundaries.
- Explain redaction, unavailable evidence, confidentiality, and information
  that cannot be shared without exposing another person or protected process.
- Make requests for more information specific, proportionate, dated, and
  recoverable when a response is late or impossible.
...
response, and direct urgent cases only to verified routes.
