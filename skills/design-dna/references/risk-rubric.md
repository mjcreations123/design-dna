...
| `RISK-PLAIN-001` | The work is human-crafted but plain: flat ground, timid scale, no imagery, no depth, no signature, presented as restraint the brief never asked for. A rendered page with zero individual defects and zero memorable elements is this failure, not a pass. | Raise the concept, not the effect count: committed palette or field, real photography at scale, depth and layering, dramatic type, one signature done fully. Verify a reviewer can name one memorable element per page. | A brief that explicitly asks for quiet, and text-first reference or archival surfaces where structure genuinely carries the work; even then the result must read authored, never unfinished. |
| `RISK-PARSE-001` | Visible strings a visitor cannot parse: decorative pseudo-data (coordinates, serials, telemetry, fake status), internal vocabulary leaking to the page, placeholder or binding residue, micro-labels whose deletion loses nothing. | Run the parseable-text four-question gate (meaning, truth, audience, deletion cost) on every rendered string in every state; delete, bind to real data, or fold into the adjacent heading. | Genuine technical subjects where every mark is true and sparse; authored marginalia that rewards reading with a checkable fact. |
| `RISK-FONTPAINT-001` | The declared face is not the painted face: a CDN or CSP failure, family-name or weight mismatch, unicode-range mistake, specificity bug, or synthesized bold ships the fallback as the identity while every loading API looks clean. | Run the full rendered-font verification in the typography reference: registration, paint, synthesis, network, console, computed-size, and fallback-rehearsal proofs; fix the loading or CSS cause. | None. A deliberately chosen system stack for dense Operate UI, product continuity, or regulated delivery is a recorded decision, not a silent failure. |
