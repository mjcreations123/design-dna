...
This guide describes the `13.3.1` development candidate. The version label is
...
```text
$design-dna Build this website using only recorded, measured references qualified for its real brief, plus the supplied facts and assets.
```
...
selection, industry-tag convenience, and submission-feed padding do not
qualify. Add Showcase only when the brief expressly calls for premium,
...
```text
$design-dna Use Showcase to build [business or product] from fully recorded, measured, brief-qualified references and the supplied facts and assets. Do not add any producer-authored visual decision.
```
...
for [business or product]. Research current award- or editor-curated sites,
visit each candidate, and retain only references whose audience, business
model, customer tasks, content topology, routes, states, material, constraints,
and register fit this brief. Reproduce the selected references' recorded visual
relationships without adding your own design, and show me the rendered wide
and narrow direction proof before scaling it. If the direction is ugly or
unconvincing, reject that reference combination and research a stronger one.
```
...
| **Standard** | A new route, meaningful feature, or ordinary redesign. | Reference provenance for every visible decision, the full dossier/recording/measurement requirement, the project gate, rendered review, and engineering verification. |
| **Enterprise Candidate** | Every fresh public website unless the task is an explicitly bounded repair or non-public surface. | Brief-qualified public-reference selection, whole-site recordings and measurements, component-level source mapping, key-state finish, whole-corpus natural copy and numeric-rhetoric integrity, and rendered wide/narrow closure. It does not claim enterprise scale or require a large site map. |
...
| **Showcase** | The brief expressly asks for premium, showcase, or high-ambition work; a rejected direction needs recovery; or the owner explicitly asks for that deeper direction challenge. High visibility or owner sensitivity alone does not select it. | Deeper brief-qualified reference research and directly reviewable contrast sufficient to challenge the first source combination; build full alternatives when uncertainty, stakes, or owner choice justify them; then complete stronger visual proof and adversarial review. It never permits producer-authored visual fill. |
| **Project Contrast** | An unrelated public build must feel materially unlike recent studio work, or the owner says sites feel alike. | A different brief-fit reference combination before broad implementation, the smallest owner-authorized closest-sibling comparison when available, and a wide/narrow collision review. It does not rotate visual ingredients arbitrarily. |
| **Direction Challenge** | An owner explicitly escalates recurrence, or deliberately asks for a high-ambition greenfield challenge. | Three or more incompatible reference-backed roots; two cross-root wide/narrow proof slices that bind source mappings, material posture, and actual asset use; a selected-versus-rejected rendered comparison; and an independent unprimed view. It must reach reviewed before broad implementation. |
| **Range Study** | A real multi-route site must demonstrate meaningful creative range. | Dependable truth, navigation, accessibility, and operations; a route-family record mapping route-owned visual compositions to references; real-path checks; and a matched route atlas. |
...
The legacy `quick` identifier is restricted to exact nonvisual mechanical
repairs. Standard is the base preset for every visible decision. Enterprise Candidate applies
...
research and record a different brief-fit reference combination that changes
the opening or content/body operation. Compare only the smallest
owner-authorized closest-sibling evidence after that work exists. At review,
test whether the result is still too close after subject nouns, dominant media,
accent, and motion are mentally removed. If it is, reject the overlapping
reference combination and research another; do not invent a replacement.
```
...
reference-backed roots whose selected sites each qualify for this brief. Bind
wide and narrow proof slices and component sources from two different roots,
select one against a rendered rejected root, and freeze an independent unprimed
view. Do not treat changed fonts, colors, imagery, shapes, or effects inside one
composition as different roots, and do not invent any root from producer taste.
```
...
path and a body whose content and structure follow that route's job and map to
recorded reference pages with comparable jobs. Keep truth, navigation meaning,
accessibility, and operations dependable across the family. Record the family in
...
Before presenting any website result, run the single project gate against the
authoritative route manifest for the exact immutable running build:
...
evidence. Quote the emitted `GATE PASS ...` or `GATE FAIL ...` verdict
verbatim. A failure blocks presentation; if it did not run, report "the gate
did not run." Keep `-B` on every in-tree Python invocation so validation does
not leave forbidden `__pycache__`, `.pyc`, or `.pyo` artifacts in the skill or
project.

`--phase first-screen` proves only the mapped first-screen checkpoint. It is a
build authorization, not post-build browser QA, and it fails as evidence for a
finished prototype when the brief calls for multiple routes or a complete
site. Before presentation, the final gate must generate and validate every
route/state at wide, narrow, and derived short-height conditions, with complete
primary experience paths and no clipping, collision, fixed-rail, hidden/dead
control, overlay inertness/stacking/focus, ARIA-state, keyboard, reduced-motion,
deep-link, reload, or dead-end finding. Code and DOM inspection find candidate
routes, controls, states, hooks, assets, and interactions; live rendering is
the authority that clears them.

The final provenance pass also rejects substitutions that erase the selected
source: moving or interactive reference media cannot become a static image,
and no font, mark, palette, layout, control, or behavior may appear without an
exact current observation/state/component source. Public copy must address the
visitor; builder notes, implementation instructions, placeholder prose, and
design narration are blocking residue. A prior project’s source record must be
revisited and regenerated against current URL/content bytes, producer/runtime
hashes, full relevant traversal, states, recording, brief qualification, and
the current manifest before reuse.

No shortcut label changes this sequence. “Quick,” demo, sample, small, hurry,
time pressure, token pressure, and cost may reduce only the agreed delivered
scope; they never reduce source qualification, traversal, recording quality,
route/state coverage, responsive/accessibility review, or either gate.
...
product] using only supplied or approved facts and assets. Admit only award-
or editor-curated source pools, visit each candidate website and its relevant
inner pages, and record why it fits this project's audience, business model,
customer tasks, content topology, routes, states, material, constraints, and
register. Reject weak, random, inaccessible, or merely industry-tagged picks.
Select and map a strong combination of recorded, measured references; every
visible layout, typeface, color, control, spacing rule, motion, divider,
navigation pattern, footer, card, and mark must name its source. Preserve a
reversible checkpoint, prove wide and narrow direction slices, then implement
the real routes and states without adding producer-authored design. Inspect the
exact rendered build across relevant widths, content, inputs, preferences, and
failure conditions. If it fails visually, reject the source combination and
research a better one. Rerun every affected check, run the one-command project
gate across every route, quote its verdict, and report unperformed checks and
remaining limits.
```
...
materially different, brief-qualified reference combinations against this
project's real content and owner requirements. Replace the failed source
combination rather than adding detached effects or inventing a new visual
system, preserve truth and working behavior, and show me the exact rerendered
...
Build visual references come from live, publicly usable websites in the
maintained registry whose source curation and individual quality clear the
eligibility floor. Then qualify each candidate for this exact brief by audience,
business model, customer task, content topology, routes and states, material,
constraints, and register. Project assets, culture, editorial work, physical
material, and owner examples can inform those criteria but cannot authorize an
unsourced visual component. Record the selected site's relevant pages,
behaviors, measured values, transferable relationships, and copying limits.
Never pick randomly, average references into a fashionable template, treat an
industry tag as fit, or treat a gallery as usability proof.
...
## Understand reference proof

Research proves that a selected source combination is both strong and suitable;
it does not satisfy a count quota. Candidates are materially different when
they offer different recorded structures or behaviors that could serve the
brief. Changing isolated fonts, colors, media, or effects inside an unchanged
producer-authored system is not an alternative.
...
another representative artifact copied from and bound to the relevant recorded
reference evidence. Use the form and fidelity that settles the uncertainty,
keep compared conditions controlled, and label anything unrendered or untested
honestly.
...
Carry the selected result in a component-source map and observable decision
ledger. These records describe what the build copies from each reference and
which measured values it reproduces; they do not grant freedom to add a hero,
signature device, typeface, page form, or interaction the evidence does not
show.
...
with its recorded component sources and measured values, inspect the relevant
responsive and state matrix, run the Design DNA source and rendered review,
fix confirmed causes, rerun affected checks, and list limitations. Reject every
visible unsourced component and any source whose fit was not established for
this brief.
```
